...
export function ReverendChatWindow({ onMessageRead }: ChatWindowProps) {
  const [sessions, setSessions] = useState<ChatSession[]>([]);
  const [unreadCounts, setUnreadCounts] = useState<Record<number, number>>({});
  const user = useUserStore((state) => state.user);
  const isReverendFather = useUserStore((state) => state.isReverendFather);

  // const isReverend = useUserStore((state) => state.isReverendFather);
  const router = useRouter();

  useEffect(() => {
    if (!user) return;
    if (!isReverendFather) return;
    // Load active chat sessions
    const loadSessions = async () => {
      try {
        const token = getCookie('token');
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/reverend/chat-sessions/active`, {
          headers: {
  ...
  const handleSessionClick = async (session: ChatSession) => {
    router.push(`/reverend/chat/${session.id}`);
